...
RSpec.describe RuboCop::Cop::Style::RedundantRegexpEscape, :config do
  context 'when EnforcedStyle is omit_redundant_escape' do
    let(:cop_config) { { 'EnforcedStyle' => 'omit_redundant_escape' } }
  ...
  context 'when EnforcedStyle is require_escape' do
    let(:cop_config) { { 'EnforcedStyle' => 'require_escape' } }

    context 'with a single-line `//` regexp' do
      context 'with unescaped non-meta char' do
        it 'does not register an offense' do
          expect_no_offenses('foo = /a/')
        end
      end

      context "with an unescaped '+' inside a character class inside a group" do
        it 'registers an offense and corrects' do
          expect_offense(<<~RUBY)
            foo = /([+])/
                     ^^ Escape is required for clarity and consistency
          RUBY
      ...
      context "with an unescaped '-' character being the last character inside a character class" do
        context 'with a regexp %r{...} literal' do
          it 'registers an offense and corrects' do
            expect_offense(<<~RUBY)
              foo = %r{[0-9-]}
      ...
      context "with an unescaped '-' character being the first character inside a character class" do
        context 'with a regexp %r{...} literal' do
          it 'registers an offense and corrects' do
            expect_offense(<<~RUBY)
              foo = %r{[-0-9]}
